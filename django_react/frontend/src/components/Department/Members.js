...
function gDM(department_id){
    const [members, setMembers] = useState([])
  
    useEffect(() => {
        const str = "/api/department/" + department_id + "/members"
        axios.get(str)
            .then(res =>{
                console.log(res)
                setMembers(res.data)
...
function DepartmentMembers() {
    ...
    const members = gDM(department_id);
                          ...
                          <Grid item className={classes.paper} container xs={12} lg={12}>
                            ...
                            <Grid item container xs={4} lg={5}>
                              <Grid item xs={12} lg={12}>
                                {member.user} {bull} {member.department} {/* Seth this is where I need you to go from member>user>profile>profile.name please*/}
                              </Grid>
                              <Grid item xs={12} lg={12}>
